...
class CargoStation extends Building {
  ...
  createRouteDirections(routePointsList) {
    ...
    console.log(!importBldData.itemType, importBldData.itemType === exportStItem);
    let routeAvailable;
    switch (importBldData.buildingCategory) {
      case "inOut1":
        routeAvailable = !importBldData.itemType || importBldData.itemType === exportStItem;
        break;
      case "inOut2":
        routeAvailable =
          !importBldData.firstMatName ||
          !importBldData.secondMatName ||
      ...
      case "inOut3":
        routeAvailable =
          !importBldData.firstMatName ||
          !importBldData.secondMatName ||
          !importBldData.thirdMatName ||
          importBldData.firstMatName === exportStItem ||
          importBldData.secondMatName === exportStItem ||
          importBldData.thirdMatName === exportStItem;
        break;
      case "storage":
        routeAvailable = !importBldData.itemTypeOutput || importBldData.itemTypeOutput === exportStItem;
        break;
    }
    ...
    const waitingInterval = setInterval(() => {
      ...
      if (
        ...
        if (exportStData.cargoStationType == "Export") {
          ...
          allItems.find((item) => {
            if (item.name == itemName) resourceMarkerImg.src = item.imageSrc;
          ...
          if (index < directionsList.length) {
            ...
            setTimeout(() => {
              ...
              if (importBldData.buildingCategory == "inOut3" || importBldData.buildingCategory == "inOut2") {
                ...
                importBldData.itemAmountOutput = addResAmount(importBldData.itemAmountOutput);
                importBldData.itemTypeOutput = currentItem;
              } else {
